...
enum class ExecutionMode {
     ...
     *
     * 계측하지 않는 것은 JUDGE 와 같지만, **판정이 되지 않는다.** 결과는 판정 큐가 아니라
     * 트라이얼 결과 큐로 가고, 제출 이력에도 역량 증거에도 남지 않는다.
     *
     * 임대도 fencing 도 없다. 잃어버리면 사용자가 다시 누르면 되는 실행이라, 정확히 한 번을
     * 보장하려고 오케스트레이터를 거칠 이유가 없다 — 그 기계장치는 판정이 두 번 기록되는
     * 것을 막으려고 있는 것이고, 여기에는 기록될 판정이 없다.
     */
    TRIAL,
     ...
     *
     * TRACE 처럼 계측하되 두 가지가 다르다. **케이스를 고르지 않는다** — 사용자가 직접
     * 적은 입력이 그대로 돈다 (TRACE 는 첫 공개 케이스로 제한한다). 그리고 **실제 출력을
     * 싣는다** — 비교 경기장은 두 풀이가 무엇을 내놓았는지까지 나란히 보여야 한다.
     *
     * 숨은 입력이 새는 길이 아니다. 여기 실리는 입력은 사용자가 적은 것이고, 여기서 도는
     * 참조 풀이는 그 문제를 이미 맞힌 사람에게만 열린다 (FR-214).
     */
    LAB,
}
