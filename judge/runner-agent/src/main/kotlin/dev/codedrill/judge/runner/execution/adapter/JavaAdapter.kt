...
class JavaAdapter : RuntimeAdapter {
    ...
    private fun drill(mode: ExecutionMode): String = if (!mode.instrumented()) {
        ...
        buildString {
            ...
            appendLine("    private static int callerLine() {")
            appendLine("        for (StackTraceElement f : new Throwable().getStackTrace()) {")
            appendLine("            if (\"Solution.java\".equals(f.getFileName())) return f.getLineNumber();")
            appendLine("        }")
            appendLine("        return 0;")
            appendLine("    }")
            appendLine()
