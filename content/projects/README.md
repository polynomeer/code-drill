...
| probe-room | 공개 테스트가 못 잡는 오답이 둘은 있다. 결함 검출은 사용자가 더 쓴 테스트의 몫만 세므로, 공개 테스트가 다 잡는 문제는 그 역량을 재지 못한다 |
...
| 결함 검출 (`DEFECT_DETECTION`) | 시험 — 더 쓴 테스트를 **참조 구현과 `mutants/` 위에서** 돌린다. 참조에서 통과하고 오답의 절반 이상을 떨어뜨렸으면 성공, 참조에서 떨어지면 틀린 것을 기대하는 테스트라 실패. 카탈로그에 달지 않는다; 더 쓰지 않았으면 증거가 없다. 그래서 `mutants/` 는 검증의 도구만이 아니라 **채점의 일부**다 — 오답이 흔한 실수를 닮을수록 이 증거가 뜻을 갖는다 |

결함 검출의 시험판은 오케스트레이터가 `probes/<packageDigest>.json` 으로 올린다 — 시작 저장소 위에
참조와 오답을 각각 덮은 완성된 워크스페이스들이다. `tamper--*` 는 빼는데, 그것은 테스트가 아니라
가드가 잡는 것이기 때문이다. Runner 는 사용자의 **테스트 파일만** 그 위에 얹어 돌리고 이름과 결과만
