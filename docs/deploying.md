...
여러 노드에 걸치는 매니페스트(Kubernetes 든 무엇이든)가 없다. 지금 서 있는 것은 **단일
호스트 compose** 다 — 앱 스택 한 벌과 Runner 노드 하나. 그 이상으로 늘릴 때가 매니페스트가
필요해지는 때이며, 워크로드 신원(A4)과 컴파일 격리(A5)도 그때 함께 푼다.
