...
`INT`, `INT_ARRAY`, `STRING`, `STRING_ARRAY`, `INT_MATRIX`. 하네스가 이 타입으로 사용자
함수를 부르고 기대 출력과 견준다.
...
`INT_MATRIX` 는 `<행>,<열>,<행 우선 원소>` 로 싣는다. 행 구분자를 새로 만들지 않으므로
기존 파서가 그대로 쓰이고, 크기를 앞에 두면 **들쭉날쭉한 격자를 표현할 수 없다** — 격자는
직사각형이라는 약속이 형식 자체에 들어간다. 크기가 없으면 `[]` 와 `[[], [], []]` 이 같은
값이 되고, 행과 열을 맞바꾼 구현이 정사각형에서 그대로 통과한다.
...
| 실무군 (ENGINEERING) | 프로젝트형 문제만 증거를 쌓는 역량군. 요구사항 충실(판정), 테스트 작성(활동), 결함 검출(더 쓴 테스트를 참조·대표 오답 위에서 시험). 알고리즘 카탈로그는 달 수 없고 프로젝트 카탈로그는 이것만 단다 — 두 판정기의 증거가 한 칸에 섞이지 않게 (11단계) |
...
컴파일도 샌드박스 안에서 돈다 (§5.5). Runner 에는 컴파일러가 없다 — Java 는 런타임
이미지(JDK)의 `javac`, Python 은 인터프리터, Kotlin 은 Runner 가 자기 클래스패스의 컴파일러
jar 를 읽기 전용으로 들여보낸다. 관리자 토큰은 설정에 든 장기 비밀이라, Identity 모듈이
