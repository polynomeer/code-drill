...
| 콘텐츠·환경을 바꿨는데 테스트가 그대로 초록이다 | 빌드 캐시(`org.gradle.caching=true`)가 옛 결과를 되살렸다. 환경 변수(`CODEDRILL_REDIS_URL` 등)와 `content/` 는 테스트 입력이 아니라서다. 믿어야 하는 실행은 `--no-build-cache` 로 돌린다 — 힌트 규칙 위반 하나와 건너뛴 Redis 테스트 열 개가 그렇게 초록으로 보인 적이 있다 |
