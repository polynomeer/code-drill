...
| 모든 제출이 SYSTEM_ERROR 고 사유가 "테스트 번들" | Runner 가 오브젝트 스토어에 못 붙거나 버킷이 없다 (§8.3). `STORAGE_ENDPOINT` 와 `up.py` 가 만든 `codedrill` 버킷을 본다. 스토어 없이 띄우면 둘 다 같은 머신의 디렉터리로 내려간다 |
...
| 콘텐츠·환경을 바꿨는데 테스트가 그대로 초록이다 | Gradle 이 그 파일들을 테스트의 입력으로 모른다. 두 갈래다 — 캐시(`org.gradle.caching=true`)가 옛 결과를 꺼내 오거나, 작업이 아예 `UP-TO-DATE` 로 건너뛴다. **`--no-build-cache` 는 앞엣것만 막는다.** `content/` 를 읽는 `:judge:runner-agent`·`:control-plane:coaching` 의 테스트는 이제 `inputs.dir` 로 선언해 두었으니 콘텐츠를 고치면 다시 돈다. 그 밖의 것(환경 변수 `CODEDRILL_REDIS_URL` 등)을 바꿨고 그 실행을 믿어야 하면 `--rerun-tasks` 다 — 힌트 규칙 위반 하나가 로컬에서 초록이고 CI 에서 빨갛게 나온 적이 있다 |
